...
**在线使用 / Open online:** https://fonteditor.692718.xyz/

**单文件版 / Single-file version:** [字体编辑工具_完整修复版.html](./字体编辑工具_完整修复版.html)

## 简体中文

一款在浏览器里使用的字体编辑工具。打开在线版即可使用；也可以下载上面的 HTML 文件，在本地浏览器打开。

- 调整整套字体或指定字符的大小、粗细、字距、上下位置和颜色。
- 修改字符的字形。
- 从多个字体中挑选字符融合到目标字体，或在同一字体内互换字符；可只处理数字、英文，或按文字类别筛选，例如只融合日语平假名、片假名而不替换汉字。
...
A browser-based font editor. Use the [online version](https://fonteditor.692718.xyz/) or download the single HTML file above and open it locally.

- Adjust size, weight, spacing, vertical position, and color for the entire font or selected characters.
...
ブラウザーで使えるフォント編集ツールです。[オンライン版](https://fonteditor.692718.xyz/)を開くか、上の HTML ファイルをダウンロードしてローカルで使用できます。

- フォント全体または指定した文字のサイズ、太さ、字間、上下位置、色を調整。
- 文字の字形を編集。
- 複数のフォントから文字を選んで統合、または同じフォント内で文字を入れ替え。数字・英字だけを対象にしたり、漢字を置き換えずに日本語のひらがな・カタカナだけを統合したりできます。
...
브라우저에서 사용하는 글꼴 편집 도구입니다. [온라인 버전](https://fonteditor.692718.xyz/)을 열거나 위의 HTML 파일을 내려받아 로컬에서 사용할 수 있습니다.

- 글꼴 전체 또는 특정 글자의 크기, 굵기, 자간, 위아래 위치, 색상을 조절합니다.
- 글자의 글리프 모양을 편집합니다.
